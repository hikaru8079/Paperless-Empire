...
public class RegistrationController : Controller{
    ...
    public async Task<IActionResult> Complete(RegistrationModel Model,IConfiguration _configuration){
        ...
        */
        ...
        var tokenaz = _configuration.GetConnectionString("SLACK_TOKEN");

        string channel = "#slackapi-test";
        var text = $"【公欠届承認案内】\n新着の公欠届があります。\n\n----概要----\n学籍番号: {model!.Number}\n氏名: {model.Name}\n公欠日: {model.Date}\n\n詳細をこちらのURLから承認ページへアクセスし、承認/却下操作を行ってください。\nhttps://paperless-empire.site/TeachingStaff";
        var data = new StringContent($"token={tokenaz}&channel={channel}&text={text}",Encoding.UTF8,"application/x-www-form-urlencoded");
